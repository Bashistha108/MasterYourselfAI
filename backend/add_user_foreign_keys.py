#!/usr/bin/env python3
"""
Add user_id foreign keys to all tables for user isolation
"""
        ...
        for table in tables_to_update:
            try:
                ...
                if 'user_id' not in columns:
                    # Add user_id column
                    db.session.execute(f"ALTER TABLE {table} ADD COLUMN user_id INTEGER")
            ...
            for table in tables_to_update:
                try:
                    ...
                    if count > 0:
                        ...
                        db.session.execute(f"UPDATE {table} SET user_id = {default_user.id} WHERE user_id IS NULL")
