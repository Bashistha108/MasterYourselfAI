...
class Config:
    ...
    if DATABASE_URL:
        ...
        print(f"Using Render PostgreSQL database with psycopg3")
    elif all([POSTGRES_HOST, POSTGRES_USER, POSTGRES_DATABASE]):
        # Manual PostgreSQL configuration
        from urllib.parse import quote_plus
        encoded_password = quote_plus(POSTGRES_PASSWORD or '')
        SQLALCHEMY_DATABASE_URI = f"postgresql+psycopg://{POSTGRES_USER}:{encoded_password}@{POSTGRES_HOST}:{POSTGRES_PORT}/{POSTGRES_DATABASE}"
